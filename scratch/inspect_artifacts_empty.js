...
findPort(async (err,list)=>{
  ...
  const clicked=await evalFn(`(() => {
    ...
    const t=btns.find(b=> (b.innerText||'').includes('交付件列表'));
    if(!t) return 'not found';
    t.click();
    return 'clicked ' + t.innerText.slice(0,50);
  })()`);
  console.log(clicked);
  await new Promise(r=>setTimeout(r,1000));
  const info=await evalFn(`(() => {
    const el=[...document.querySelectorAll('*')].find(e=> (e.textContent||'').trim()==='No artifacts generated');
    if(!el) {
      const fuzzy=[...document.querySelectorAll('*')].find(e=> (e.textContent||'').includes('No artifacts'));
      if(!fuzzy) return {found:false};
      return {found:'fuzzy', tag:fuzzy.tagName, cls:(fuzzy.className||'').toString().slice(0,100), text:(fuzzy.textContent||'').slice(0,100), html:fuzzy.outerHTML.slice(0,500)};
    }
    let cur=el;
    const chain=[];
    for(let i=0;i<6&&cur;i++){
      chain.push({tag:cur.tagName, cls:(cur.className||'').toString().slice(0,100), translate:cur.getAttribute('translate')||'', notranslate:cur.classList.contains('notranslate')});
    ...
    let cur2=el;
    const detailed=[];
    for(let i=0;i<8&&cur2;i++){
      detailed.push({tag:cur2.tagName, cls:(cur2.className||'').toString().slice(0,80), hasBlocking:hasBlocking(cur2)});
      cur2=cur2.parentElement;
    }
    return {found:true, tag:el.tagName, cls:(el.className||'').toString().slice(0,100), text:(el.textContent||'').slice(0,100), chain, detailed, html:el.outerHTML.slice(0,600)};
  })()`);
  console.log(JSON.stringify(info,null,2));
  // 同时检查 No uploads
  const info2=await evalFn(`(() => {
    const el=[...document.querySelectorAll('*')].find(e=> (e.textContent||'').trim()==='No uploads');
    ...
    return {found:true, tag:el.tagName, cls:(el.className||'').toString().slice(0,100), text:el.textContent, chain, html:el.outerHTML.slice(0,500)};
  })()`);
  console.log('No uploads',JSON.stringify(info2,null,2));
