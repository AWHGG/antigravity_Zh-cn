...
const keys=['Device Name','Scan the code to open this device in Remote Control, or'];
for(const k of keys){
  const n=norm(k).toLowerCase();
  const found=Object.keys(all).find(x=>norm(x).toLowerCase()===n);
  console.log(JSON.stringify(k), found? 'FOUND '+JSON.stringify(found)+' => '+JSON.stringify(all[found]).slice(0,60) : 'MISSING');
}
