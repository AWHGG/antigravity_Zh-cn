...
> [!IMPORTANT]
> **平台支持说明**：本项目目前**仅支持 Windows 系统**（Windows 10 / 11），**不支持 macOS 与 Linux 系统**。请勿在 macOS 或 Linux 环境下运行安装脚本。
...
| 平台 / 环境 | 支持状态 | 说明 |
| :--- | :--- | :--- |
| **Windows 10 / 11 (x64 / ARM64)** |  **支持** | 提供一键 `.bat` 交互脚本，自动探测注册表、默认目录及后台进程 |
| **macOS (Apple Silicon / Intel)** | ❌ **不支持** | 暂未适配 macOS 应用打包架构与系统路径，请勿在 macOS 上运行 |
| **Linux (deb / rpm / AppImage)** | ❌ **不支持** | 暂未适配 Linux 客户端结构 |
| **Node.js** | >= 18.0.0 | 运行自动化测试门禁或使用命令行高级用法时需要 |
