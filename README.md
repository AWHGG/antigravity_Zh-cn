...
| **`维护工具.bat`** | 包含 6 种维护与质量诊断脚本的交互菜单，用于漏译排查与字典校验。 |
...
| `engine_fix_test.js` | 运行 179 项自动化回归测试，涵盖源文件结构完整性、JS 语法、jsdom 真实 DOM 行为、主进程桩行为、动态句式、属性漏译采集、禁区零开口（禁区内输入框属性全路径不翻译）、品牌模式运行期全链路、asar 头解析与状态机判定。 |
